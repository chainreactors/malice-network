...
func RegListKey(rpc clientrpc.MaliceRPCClient, session *session.Session, hive, path string) (*clientpb.Task, error) {
...
func RegisterRegListFunc(con *repl.Console) {
	...
	con.RegisterImplantFunc(
		...
		RegListValue,
		"breq_query",
		func(rpc clientrpc.MaliceRPCClient, sess *session.Session, key, arch string) (*clientpb.Task, error) {
...
func RegListValue(rpc clientrpc.MaliceRPCClient, session *session.Session, hive, path string) (*clientpb.Task, error) {
	request := &implantpb.RegistryRequest{
