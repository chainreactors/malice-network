...
日志内容取决于构建源：Docker 持久化容器标准输出和标准错误；SaaS 不同步远端编译器输出，只持久化一次终态日志。

| SaaS 状态 | 日志标记 | 语义 |
|-----------|----------|------|
| `completed` | `[COMPLETED]` | 构建、下载和本地保存均成功 |
| `failure` | `[FAILED]` | 远端明确失败，或发生非网络请求、本地处理错误 |
| `networkerr` | `[NETWORK_ERROR]` | 提交、状态查询或下载发生网络错误，构建结果可能无法确认 |

SaaS 失败日志通过 `stage=submit|status|download` 标识失败阶段。状态轮询的临时网络错误不会立即结束构建；轮询恢复后继续等待，持续到超时仍不可达时才写入 `networkerr`，并保留最后一次网络错误。终态状态与对应日志在同一次数据库更新中写入。
