...
`cache/cache.bin` 是可重建的任务结果加速索引，不是任务结果的唯一持久化来源。任务结果文件和
数据库记录仍是恢复依据；如果 `cache.bin` 损坏，Server 会忽略它并从任务数据继续恢复。缓存
使用原子替换落盘，每个 Session 删除时也会注销该缓存拥有的定时保存和清理任务，避免后台任务
继续持有已结束的 Session。
